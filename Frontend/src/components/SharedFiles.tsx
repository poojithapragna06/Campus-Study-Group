...
import { sharedFiles, studyGroups } from '../data/mockData';
...
const SharedFiles: React.FC = () => {
  ...
  const handleDrop = (e: React.DragEvent) => {
    ...
    const newFiles: SharedFile[] = droppedFiles.map((f, i) => ({
      id: 'f' + Date.now() + i, groupId: 'g1', name: f.name,
      size: (f.size / 1024 / 1024).toFixed(1) + ' MB', type: f.type.includes('image') ? 'image' : 'pdf',
      uploadedBy: 'Aryan Sharma', uploadedAt: new Date().toISOString().split('T')[0], url: '#'
    }));
  ...
  return (
    <div className="p-8 animate-[slideIn_0.3s_ease-out]">
      ...
      >
        ...
        <p className="text-xs mt-1" style={{ color: '#4A5A70' }}>PDF, Images, ZIP — Max 25MB</p>
      </div>
      ...
      <div className="rounded-2xl overflow-hidden" style={{ border: '1px solid #1E2A3A' }}>
        ...
        <div>
          {filtered.length === 0 ? (
            ...
            return (
              <div key={file.id} className="grid grid-cols-12 gap-4 items-center px-5 py-4 transition-colors duration-150 hover:opacity-80"
                style={{ background: i % 2 === 0 ? '#1A1F2E' : '#161D2A', borderTop: '1px solid #1E2A3A' }}>
                ...
                <div className="col-span-1 flex items-center gap-1 justify-end">
                  <button className="p-1.5 rounded-lg transition-colors hover:opacity-80" style={{ background: '#1E2A3A', color: '#6B7A8D' }}><Download size={12} /></button>
                  <button onClick={() => setFiles(p => p.filter(f => f.id !== file.id))} className="p-1.5 rounded-lg transition-colors hover:opacity-80" style={{ background: '#1E2A3A', color: '#EF4444' }}><Trash2 size={12} /></button>
                </div>
              </div>
            );
          })}
