...
import { studyGroups, users, messages, sharedFiles } from '../data/mockData';

const AdminPanel: React.FC = () => {
  const [activeTab, setActiveTab] = useState<'overview' | 'groups' | 'users' | 'content'>('overview');
  const [groups, setGroups] = useState(studyGroups);
  const [allUsers, setAllUsers] = useState(users);

  const stats = [
    { label: 'Total Users', value: users.length, icon: Users, color: '#FFB800', delta: '+12%' },
    { label: 'Active Groups', value: studyGroups.filter(g => g.status === 'active').length, icon: BookOpen, color: '#00D4AA', delta: '+3%' },
    ...
    <div className="p-8 animate-[slideIn_0.3s_ease-out]">
      ...
      {activeTab === 'overview' && (
        <div className="grid grid-cols-2 gap-6">
            ...
            <div className="space-y-3">
              {groups.slice(0, 5).map(g => (
                <div key={g.id} className="flex items-center gap-3">
                  <div className="w-6 h-6 rounded-lg flex-shrink-0" style={{ background: g.coverColor + '30' }}>
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="h-1.5 rounded-full mt-1 overflow-hidden" style={{ background: '#2A3A50' }}>
                      <div className="h-full rounded-full transition-all" style={{ width: `${(g.members.length / g.maxMembers) * 100}%`, background: g.coverColor }} />
                    </div>
                  </div>
          ...
          <div className="rounded-2xl p-5" style={{ background: '#1A1F2E', border: '1px solid #1E2A3A' }}>
            ...
            <div className="space-y-3">
              {[
                { type: 'Spam', group: 'Algorithms & DS', user: 'Anonymous', time: '2h ago', severity: 'low' },
                { type: 'Inappropriate Content', group: 'Calculus III', user: 'u3', time: '5h ago', severity: 'medium' },
                { type: 'Off-topic Discussion', group: 'Quantum Physics', user: 'u5', time: '1d ago', severity: 'low' },
              ].map((r, i) => (
          ...
          {groups.map((group, i) => (
            <div key={group.id} className="grid grid-cols-12 gap-4 items-center px-5 py-4" style={{ background: i % 2 === 0 ? '#1A1F2E' : '#161D2A', borderTop: '1px solid #1E2A3A' }}>
              ...
              <div className="col-span-2 flex gap-2">
                <button onClick={() => setGroups(p => p.map(g => g.id === group.id ? { ...g, status: g.status === 'active' ? 'archived' : 'active' } : g))}
                  className="p-1.5 rounded-lg" style={{ background: '#1E2A3A', color: '#FFB800' }}>
                  <Ban size={12} />
                </button>
                <button onClick={() => setGroups(p => p.filter(g => g.id !== group.id))} className="p-1.5 rounded-lg" style={{ background: '#1E2A3A', color: '#EF4444' }}>
                  <Trash2 size={12} />
                </button>
              </div>
            </div>
          ...
          {allUsers.map((user, i) => (
            <div key={user.id} className="grid grid-cols-12 gap-4 items-center px-5 py-4" style={{ background: i % 2 === 0 ? '#1A1F2E' : '#161D2A', borderTop: '1px solid #1E2A3A' }}>
              ...
              <div className="col-span-2 flex gap-2">
                ...
                <button onClick={() => setAllUsers(p => p.filter(u => u.id !== user.id))} className="p-1.5 rounded-lg" style={{ background: '#1E2A3A', color: '#EF4444' }}><Trash2 size={12} /></button>
              </div>
            </div>
          ))}
        </div>
      ...
      {activeTab === 'content' && (
        <div className="space-y-4">
          <div className="p-5 rounded-2xl" style={{ background: '#1A1F2E', border: '1px solid #1E2A3A' }}>
              ...
              {messages.slice(0, 3).map(msg => (
                <div key={msg.id} className="flex items-start gap-4 p-4 rounded-xl" style={{ background: '#111827' }}>
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-sm" style={{ color: '#6B7A8D' }}>{msg.type === 'file' ? `Shared file: ${msg.fileName}` : msg.content}</p>
                  </div>
