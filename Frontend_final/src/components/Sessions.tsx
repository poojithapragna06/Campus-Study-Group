...
import { Calendar, Clock, MapPin, Users, Plus, X, Video } from 'lucide-react';
import { currentUser } from '../data/mockData';
import { useSessions, useCreateSession, useRsvpSession } from '../hooks/useQueries';
...
const Sessions: React.FC = () => {
  ...
  const rsvpSession = useRsvpSession();
  ...
  const getGroupColor = (id: string) => groups.find(g => g.id === id)?.coverColor ?? '#FFB800';

  const handleCreate = () => {
    createSession.mutate(
      { ...form, attendees: [currentUser.id], maxAttendees: 10 },
      { onSuccess: () => { setShowCreate(false); setForm({ title: '', groupId: '', date: '', time: '', duration: 60, location: '', isOnline: false, description: '' }); } }
    );
  };

  const handleRsvp = (sessionId: string) => {
    rsvpSession.mutate({ sessionId, userId: currentUser.id });
  };
          ...
          <div className="mb-8">
            ...
            {upcoming.length === 0 ? (
              ...
              <div className="space-y-3">
                {upcoming.map(s => (
                  <SessionCard key={s.id} session={s} getGroupName={getGroupName} getGroupColor={getGroupColor}
                    onRsvp={() => handleRsvp(s.id)} rsvpPending={rsvpSession.isPending && rsvpSession.variables?.sessionId === s.id} />
                ))}
              </div>
            )}
          </div>
          ...
          {past.length > 0 && (
            <div>
              ...
              <div className="space-y-3 opacity-60">
                {past.map(s => (
                  <SessionCard key={s.id} session={s} getGroupName={getGroupName} getGroupColor={getGroupColor} past />
                ))}
              </div>
            </div>
          )}
          ...
          <div className="w-full max-w-lg rounded-2xl p-6" style={{ background: '#1A1F2E', border: '1px solid #2A3A50' }}>
            ...
            <div className="flex gap-3 mt-6">
              ...
              <MutationButton isPending={createSession.isPending} label="Schedule" pendingLabel="Scheduling…"
                onClick={handleCreate} className="flex-1 py-2.5 rounded-xl text-sm font-medium" style={{ background: '#00D4AA', color: '#0D0D0D' }} />
            </div>
          </div>
        </div>
      )}
...
const SessionCard: React.FC<SessionCardProps> = ({ session, getGroupName, getGroupColor, onRsvp, rsvpPending, past }) => {
  const isRsvpd = session.attendees.includes(currentUser.id);
    ...
    <div className="flex items-center gap-5 p-5 rounded-2xl" style={{ background: '#1A1F2E', border: '1px solid #1E2A3A' }}>
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2 mb-1">
          ...
          <span className="px-2 py-0.5 rounded-md text-xs" style={{ background: color + '20', color }}>
      ...
      {!past && (
        <MutationButton
          ...
          onClick={onRsvp}
          className="px-4 py-2 rounded-xl text-xs font-medium flex-shrink-0"
          style={{
