...
import { StudyGroup, ActiveView } from '../types';
    ...
    createGroup.mutate(
      ...
      },
      {
        onSuccess: () => {
          ...
          setNewGroup({ name: '', subject: '', description: '', isPrivate: false, maxMembers: 8 });
        },
      }
    );
  };
  ...
  return (
    <div className="p-8 animate-[slideIn_0.3s_ease-out]">
      <div className="flex items-center justify-between mb-8">
        ...
        <button onClick={() => setShowCreateModal(true)} className="flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-medium" style={{ background: '#FFB800', color: '#0D0D0D' }}>
      ...
      <div className="flex items-center gap-4 mb-6">
        <div className="flex-1 relative">
          ...
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search groups, subjects…"
            className="w-full pl-10 pr-4 py-2.5 rounded-xl text-sm outline-none"
            style={{ background: '#1E2A3A', border: '1px solid #2A3A50', color: '#E8EDF4' }} />
        </div>
        <div className="flex gap-2 flex-wrap">
          {subjects.map(s => (
            <button key={s} onClick={() => setFilterSubject(s)} className="px-3 py-2 rounded-xl text-xs font-medium transition-all duration-200"
              style={{ background: filterSubject === s ? '#FFB800' : '#1E2A3A', color: filterSubject === s ? '#0D0D0D' : '#6B7A8D', border: '1px solid ' + (filterSubject === s ? '#FFB800' : '#2A3A50') }}>
              {s}
            </button>
          ))}
        </div>
      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-3 gap-5">
          {filtered.map(group => (
            <GroupCard key={group.id} group={group} onOpen={() => { setSelectedGroup(group); setActiveView('chat'); }} />
          ))}
        </div>
      )}
          ...
          <div className="w-full max-w-md rounded-2xl p-6" style={{ background: '#1A1F2E', border: '1px solid #2A3A50' }}>
            ...
            <div className="space-y-4">
              {[
                ...
                <div key={key}>
                  ...
                  <input value={(newGroup as any)[key]} onChange={e => setNewGroup(p => ({ ...p, [key]: e.target.value }))}
              ...
              <div>
                ...
                <textarea value={newGroup.description} onChange={e => setNewGroup(p => ({ ...p, description: e.target.value }))}
                  placeholder="What will your group focus on?" rows={3}
            ...
            <div className="flex gap-3 mt-6">
              ...
              <MutationButton
                ...
                style={{ background: '#FFB800', color: '#0D0D0D', opacity: (!newGroup.name || !newGroup.subject) ? 0.5 : 1 }}
              />
            </div>
          </div>
        </div>
      ...
      <div className="p-5">
        ...
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-1.5">
            ...
            <span className="text-xs" style={{ color: '#4A5A70' }}>{group.members.length}/{group.maxMembers}</span>
          </div>
          <div className="flex items-center gap-1 text-xs" style={{ color: '#4A5A70' }}>
            <Users size={11} />{group.maxMembers - group.members.length} spots left
          </div>
        </div>

        <div className="flex gap-2">
          <button onClick={() => setJoined(!joined)} disabled={group.status === 'archived'}
            className="flex-1 py-2 rounded-xl text-xs font-medium transition-all duration-200"
            style={{ background: joined ? 'rgba(255,184,0,0.1)' : '#FFB800', color: joined ? '#FFB800' : '#0D0D0D', border: joined ? '1px solid #FFB800' : 'none', opacity: group.status === 'archived' ? 0.5 : 1 }}>
            {joined ? 'Joined ✓' : 'Join Group'}
          </button>
          <button onClick={onOpen} className="flex items-center justify-center w-9 h-8 rounded-xl"
